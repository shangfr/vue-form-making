...
def eva_pks(dfkslift, title):
    ...
    plt.gca().set(title=title+'K-S', 
      xlabel='% of Companies', ylabel='% of total Weak/Strong', 
      xlim=[0,1], ylim=[0,1], aspect='equal')
    # text
    # plt.text(0.5,0.96,'K-S', fontsize=15,horizontalalignment='center')
    plt.text(0.2,0.8,'Strong',horizontalalignment='center')
    plt.text(0.8,0.55,'Weak',horizontalalignment='center')
...
def eva_plift(dfkslift, title):
    ...
    plt.gca().set(title=title+'Lift', 
      xlabel='% of Companies', ylabel='% of Strong', 
      xlim=[0,1], ylim=[0,1], aspect='equal')
    # text
...
def perf_psi(score, label=None, title=None, x_limits=None, x_tick_break=50, show_plot=True, seed=186, return_distr_dat=False):
    ...
    PSI
    ------
    perf_psi calculates Companies stability index (PSI) and provides 
    ...
    Details
    ------
    The Companies stability index (PSI) formula is displayed below: 
    ...
    0.25 inference major shift in Companies, need to delve deeper.
