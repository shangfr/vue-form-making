...
def woebin(dt, y, x=None, 
    ...
    The default woe is defined as ln(Distr_Strong_i/Distr_Weak_i). If you 
    prefer ln(Distr_Strong_i/Distr_Weak_i), please set the argument `positive` 
...
def plot_bin(binx, title, show_iv, savefig):
    '''
    ...
    ax2.set_ylabel('Strong probability', color='blue')
    ...
    plt.legend((p2[0], p1[0]), ('Strong', 'Weak'), loc='upper right')
    if savefig is not None:
        plt.savefig('data_out\\'+binx.loc[0,'variable']+'.png')
        print('data_out\\'+binx.loc[0,'variable']+'.png')
    # show plot 
    # plt.show()
    return fig


def woebin_plot(bins, x=None, title=None, show_iv=True, savefig=None):
    '''
    ...
    for i in xs:
        ...
        plotlist[i] = plot_bin(binx, title, show_iv, savefig)
    return plotlist
